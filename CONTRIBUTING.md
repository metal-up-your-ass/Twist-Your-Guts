...
- Create a feature branch off `main`, named descriptively (e.g. `feature/lr4-crossover`, `fix/gate-hysteresis`, `docs/building-windows`).
...
```
feat(dsp): add LR4 crossover with adjustable split frequency
fix(gate): correct hysteresis threshold comparison
test(compressor): add null test for unity-gain bypass
```
...
- **Crossover flat-sum tests** — verify that band-split components sum back to unity (or the expected reference) across the audible spectrum, within tolerance.
- **Null tests** — a bypass or unity-parameter configuration must null against a known-good reference to a tight tolerance (e.g. below -90 dBFS residual).
