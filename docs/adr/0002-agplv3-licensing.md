...
* Neutral, because a future commercial/dual-licensed release remains possible: only the copyright holder(s) (i.e. the project's own contributors, collectively or via a CLA if one is later introduced) could relicense the project's own code commercially — this does not on its own resolve JUCE's licensing (a commercial JUCE license would still be needed for a closed-source relicensed build), and no such relicensing is planned in v1.0 scope.
