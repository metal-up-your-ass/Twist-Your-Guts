...
* Breadth of built-in DSP coverage (filters, oversampling, convolution, etc.) so the crossover, distortion, and IR-loader work isn't reinventing well-trodden primitives.
...
* Track record: professional, commercially shipped plugins in the same "heavy DSP, character/amp-style processing" category are built on it, suggesting the framework holds up under real-world DSP and GUI demands (Neural DSP-class plugins ship on JUCE).
...
* Good, because `juce::dsp` provides ready building blocks (IIR/FIR filters, oversampling, convolution, state variable filters) that map directly onto this project's crossover, distortion oversampling, EQ, and IR-loader needs.
